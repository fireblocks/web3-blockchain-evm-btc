...
export class EVMAdapter implements BlockchainAdapter {
  ...
  async buildUnsignedTransaction(params: TransferParams): Promise<UnsignedTransaction> {
    ...
    if (feeData.maxFeePerGas && feeData.maxPriorityFeePerGas) {
      ...
      txData.maxFeePerGas = ethers.parseUnits('50', 'gwei');
      txData.maxPriorityFeePerGas = ethers.parseUnits('2', 'gwei');
    }
    ...
    return {
      ...
      estimatedFee: {
        ...
        decimals: 18,
        displayAmount: ethers.formatEther(estimatedFeeAmount),
  ...
  async signTransaction(
    ...
    const txData = unsignedTx.raw as ethers.TransactionLike;
    ...
    const serializedTx = ethers.Transaction.from(txDataWithoutFrom).unsignedSerialized;
  ...
  async embedSignature(
    ...
    const tx = ethers.Transaction.from(txDataWithoutFrom);
  ...
  async getBalance(
    ...
    const decimals = 18;
    const displayBalance = ethers.formatUnits(balanceWei, decimals);
  ...
  async getTokenBalance(
    ...
    const displayBalance = ethers.formatUnits(balanceRaw, decimals);
  ...
  async getMaxTransferableAmount(
    ...
    try {
      ...
      const displayAmount = ethers.formatEther(maxTransferableWei);
