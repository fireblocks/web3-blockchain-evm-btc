...
function writeUInt64LE(value: bigint | number | string): Uint8Array {
...
// ---------------------------------------------------------------------------
// UTXO selection — greedy largest-first with iterative fee estimation
// ---------------------------------------------------------------------------
...
export interface SelectedUTXOs {
  ...
  fee: bigint; // satoshis
  change: bigint; // satoshis (0 = no change output)
}
...
export function selectUTXOs(
  ...
  feeRates: FeeRates,
  isSegwit: boolean,
  feeRateLevel: "fast" | "medium" | "slow" = "fast",
  minFeeRate = 1,
): SelectedUTXOs {
  ...
  for (const utxo of sorted) {
    ...
    const feeWithChange = BigInt(
      estimateFee(selected.length, 2, isSegwit, rate),
    );
    ...
    if (totalInput >= amountSat + feeWithChange) {
      ...
      if (change > 546n) return { selected, totalInput, fee, change };
    }
  }

  throw new Error(
    `Insufficient funds: need ${amountSat} sat + fees, have ${totalInput} sat across ${utxos.length} UTXOs`,
  );
}

// ---------------------------------------------------------------------------
// Unsigned transaction data stored in UnsignedTransaction.raw
// ---------------------------------------------------------------------------
...
export interface UtxoUnsignedTxRaw {
  inputs: Array<{
    ...
    sequence: number;
    scriptPubKey: string; // hex — the scriptPubKey of the UTXO being spent (needed for BIP143)
  }>;
  outputs: Array<{
    address: string;
    value: bigint; // satoshis (stored as string in JSON via sanitizeBigInt in relay)
    scriptPubKey: string; // hex
  }>;
  version: number;
  locktime: number;
  isSegwit: boolean;
  // BCH replay-protection: uses BIP143 sighash with SIGHASH_ALL|SIGHASH_FORKID (0x41)
  // instead of plain SIGHASH_ALL (0x01). Signing and serialization are still legacy.
  useForkId?: boolean;
  sighashes: string[]; // one per input, hex — the preimage each input must sign
}
...
// ---------------------------------------------------------------------------
// Address → scriptPubKey decoding (used when building outputs)
// ---------------------------------------------------------------------------

// Network parameters needed to validate a destination address actually belongs
// to *this* chain before building a transaction to it. Passing the real,
// chain-specific version bytes/HRP (rather than accepting any Bitcoin-family
// address) is what lets P2SH be recognised correctly and wrong-network/typo'd
// addresses be rejected instead of silently building a bad output.
export interface UtxoNetworkParams {
  p2pkhVersion: number;
  p2shVersion: number;
  // Allowed bech32 HRP(s) for this chain (e.g. ['bc'] for BTC mainnet). Empty
  // for chains with no native SegWit support (DOGE, DASH, BSV, BCH-legacy).
  bech32Hrps: string[];
}
...
function decodeBech32ToScript(address: string, allowedHrps: string[]): Uint8Array {
  const CHARSET = "qpzry9x8gf2tvdw0s3jn54khce6mua7l";
  const lower = address.toLowerCase();
  const sepIdx = lower.lastIndexOf("1");
  if (sepIdx < 1) throw new Error(`Invalid bech32 address: ${address}`);
  ...
  if (dataChars.length < 6) throw new Error(`Invalid bech32 address: ${address}`);
  ...
  return concat(
    new Uint8Array([opcode, witnessProgram.length]),
    witnessProgram,
  );
}
...
function decodeBase58CheckToHash(address: string): { hash: Uint8Array; versionByte: number } {
  const decoded = base58Decode(address);
  if (decoded.length < 5) throw new Error(`Invalid base58check address: ${address}`);
  ...
  if (hash.length !== 20) throw new Error(`Invalid base58check payload length for address: ${address}`);
...
export function addressToScriptPubKey(
  ...
  throw new Error(
    `Address ${address} does not match this chain's network (expected version 0x${network.p2pkhVersion.toString(16)} for P2PKH or 0x${network.p2shVersion.toString(16)} for P2SH, got 0x${versionByte.toString(16)})`,
  );
}
...
export function buildUnsignedTx(
  inputs: Array<{ utxo: UTXO; sequence?: number }>,
  ...
  if (useForkId) {
    sighashes = buildSegwitSighashes(
      txInputs,
      txOutputs,
      version,
      locktime,
...
function buildSegwitSighashes(
  inputs: UtxoUnsignedTxRaw["inputs"],
  outputs: UtxoUnsignedTxRaw["outputs"],
  version: number,
  locktime: number,
  hashType = 0x01,
): string[] {
  ...
  const prevouts = concat(
    ...inputs.map((inp) =>
      concat(reverseTxid(inp.txid), writeUInt32LE(inp.vout)),
    ),
  );
  ...
  return inputs.map((inp) => {
    ...
    let finalScriptCode: Uint8Array;
    if (
      scriptCode.length === 22 &&
      scriptCode[0] === 0x00 &&
      scriptCode[1] === 0x14
    ) {
    ...
    const preimage = concat(
      ...
      encodeVarint(finalScriptCode.length),
      finalScriptCode,
      writeUInt64LE(BigInt(inp.value)),
      writeUInt32LE(inp.sequence),
      hashOutputs,
      writeUInt32LE(locktime),
      writeUInt32LE(hashType), // SIGHASH_ALL = 0x01, SIGHASH_ALL|SIGHASH_FORKID = 0x41 (BCH)
    );
...
function buildLegacySighashes(
  ...
  return inputs.map((_signingInput, sigIdx) => {
    const serialized = concat(
      writeUInt32LE(version),
      encodeVarint(inputs.length),
      ...inputs.map((inp, i) => {
        const script =
          i === sigIdx ? hexToBytes(inp.scriptPubKey) : new Uint8Array(0);
        return concat(
          reverseTxid(inp.txid),
          writeUInt32LE(inp.vout),
      ...
      writeUInt32LE(locktime),
      writeUInt32LE(0x00000001), // SIGHASH_ALL
    );
...
// ---------------------------------------------------------------------------
// Sign each input sighash
// ---------------------------------------------------------------------------

export interface InputSignature {
  der: string; // DER-encoded signature + sighash type byte, hex
...
function serializeLegacyTx(
  raw: UtxoUnsignedTxRaw,
  signatures: InputSignature[],
): string {
  const parts: Uint8Array[] = [writeUInt32LE(raw.version)];
...
function serializeSegwitTx(
  ...
  for (let i = 0; i < raw.inputs.length; i++) {
    ...
    parts.push(
      new Uint8Array([0x02]), // item count
      encodeVarint(sig.length),
      sig,
      encodeVarint(pub.length),
      pub,
