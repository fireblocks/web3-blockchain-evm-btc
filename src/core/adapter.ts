...
export interface TransferParams {
  ...
  // Optional token information (for token transfers)
  token?: {
    contractAddress?: string; // ERC20 contract address
    mintAddress?: string;     // SPL mint address / IOU issuer
    decimals: number;         // Token decimals
    symbol?: string;          // Token symbol (e.g. "RLUSD" for XRP IOU currency code)
  };

  // Optional memo/tag/note (for blockchains that support it)
  // - Algorand: note field (up to 1KB)
  // - XRP: destination tag (numeric)
  // - Stellar: memo (text, id, hash, or return)
  // - Cosmos: memo field
  // - EOS: memo field
  memo?: string;

  // UTXO-specific multi-provider config. When present, UTXO adapters use this
  // ordered list of providers with automatic fallback. The apiKey is injected
  // at runtime from secure settings (never stored in asset configs).
  utxoRpcConfig?: {
    endpoints: Array<{
      url: string;
...
export interface UnsignedTransaction {
  ...
  raw: unknown;             // Blockchain-specific unsigned transaction data
  estimatedFee?: {          // Estimated transaction fee (for UI display)
    amount: string;         // Fee amount in base units (e.g., wei, lamports)
    decimals: number;       // Decimals for the fee currency
    displayAmount: string;  // Human-readable fee amount
    currency: string;       // Fee currency symbol (e.g., ETH, SOL)
  };
  metadata?: {              // Optional metadata for display/verification
    from?: string;
    to?: string;
    amount?: string;
    tokenAddress?: string;
...
export interface TransactionSignature {
  ...
  v?: number;               // Recovery id (ECDSA)
  raw?: string;             // Complete signature (hex)
  serializedTransaction?: string; // Complete serialized signed transaction (hex) - used by some chains like Solana
}
...
export interface SignedTransaction {
  chainId: string;          // Blockchain identifier
  type: 'native' | 'token'; // Transfer type
  raw: string;              // Serialized signed transaction (hex)
...
export interface BroadcastResult {
  ...
  raw?: unknown;            // Blockchain-specific response data
}
...
export interface BalanceInfo {
  balance: string;          // Balance in base units (e.g., wei for ETH)
  decimals: number;         // Token decimals
  displayBalance: string;   // Human-readable balance (formatted with decimals)
}
